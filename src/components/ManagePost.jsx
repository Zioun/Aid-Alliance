...
const ManagePost = () => {
  ...
  const getData = async () => {
    const { data } = await axios(
      `${import.meta.env.VITE_API_URL}/volunteers/${user.email}`,{withCredentials: true}
    );
    setVolunteer(data);
  };
  ...
  const reqgetData = async () => {
    const { data } = await axios(
      `${import.meta.env.VITE_API_URL}/be-a-volunteer/${user.email}`,{withCredentials: true}
    );
                          ...
                          <td class="px-4 py-4 text-sm whitespace-nowrap">
                            <div class="flex items-center gap-x-6">
                              <button
                                ...
                                class="text-gray-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-300 hover:text-red-500 focus:outline-none"
                              >
                                <span className="badge bg-red-500 text-white">Cancle</span>
                              </button>
                            </div>
                          </td>
                        </tr>
